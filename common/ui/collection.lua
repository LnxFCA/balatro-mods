function LNXFCA.UIDEF.generate_collection(pool, rows, args)
    args = args or {}
    ...
    if args.opt_callback then
        args.opt_callback({ cycle_config = { current_option = 1 }})
    else
        LNXFCA.callbacks.update_collection_page({ cycle_config = { current_option = 1 }})
    end
end
    ...
    return {
        n = G.UIT.C, config = { align = "cm", padding = 0.1, }, nodes = {
            ...
            { n = G.UIT.R, config = { align = "cm", }, nodes = {
                create_option_cycle({
                    options = LNXFCA.UIDEF.collection.options, cycle_shoulders = true, opt_callback = args.opt_callback,
